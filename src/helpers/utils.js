...
// Convert large numbers to abbreviations E.g. 15000 to 15K
...
// Returns an array of years starting at `s` less than current year
// and ending with the int passed into the range variable.
// `s` is either 0 (if undefined) or the int passed into the function
export const yearsArray = (range, start) => {
  let s = start === undefined ? 0 : start;

  let max = new Date().getFullYear() - s;
  let min = max - range;
  let years = [];
...
// Returns an array of numbers the length of the provided int (as count)
export const numArray = count => {
  return Array.from({ length: count }, (_, i) => i + 1);
};

export const setDate18YearsAgo = () => {
  let d = new Date();
  let y = d.getFullYear() - 18;
  let epoch = d.setFullYear(y);
...
export const getTotalPrice = (cart, baseItems) =>
  cart.reduce((accumulator, currentValue) => {
    ...
    const { loanAmount, sale } = baseItems.find(item => item.id === id);
    return accumulator + calculateSale(loanAmount, sale);
  }, 0);

export const getSubtotal = items =>
  items.reduce((acc, curr) => curr.loanAmount + acc, 0);
