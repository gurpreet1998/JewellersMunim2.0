...
import { CashCheckMoneyOrderPaymentBatchService, CashCheckMoneyOrderService } from '_services/accounting';
...
const [reconcileCheck, setreconcileCheck] = useState(true);
    ...
    useEffect(() => {
        CashCheckMoneyOrderPaymentBatchService.getPaymentBatchType().then(res => setPaymentBatchTypes(res));
      }, []);

      useEffect(() => {
        CashCheckMoneyOrderPaymentBatchService.getPaymentBatch(paymentbatchtype).then(res => setPaymentBatches(res));
      }, [paymentbatchtype]);    
    
    useEffect(() => {
            CashCheckMoneyOrderService.getGetReconciledDataForCashCheckAndMoneyOrder(paymentbatch).then(res => {
    ...
    const reconcileOnClick = event => {
        CashCheckMoneyOrderService.getGetReconciledDataForCashCheckAndMoneyOrder(event).then(res => {
          setDepositData(res.result.bankDepositDataModel);
          setLoanData(res.result.paymentDataModel);
          setBflag(true);
        });
    ...
    const unreconciledOnClick = e => {
        CashCheckMoneyOrderService.getGetUnReconciledDataForCashCheckAndMoneyOrder(e).then(res => {
          setDepositData(res.result.bankDepositDataModel);
          setLoanData(res.result.paymentDataModel);
          setBflag(false);
        });
      ...
      const matchOnClick = () => {
        CashCheckMoneyOrderService.saveMatchRecordsForCashCheckAndMoneyOrder(paymentbatch, selectedData).then(res => {
          setDepositData(res.result.bankDepositDataModel);
          setLoanData(res.result.paymentDataModel);
          let paymentSum = 0;
          let depositSum = 0;
      ...
      const unMatchOnClick = () => {
        CashCheckMoneyOrderService.saveUnMatchRecordsForCashCheckAndMoneyOrder(paymentbatch, selectedData).then(res => {
          setDepositData(res.result.bankDepositDataModel);
          setLoanData(res.result.paymentDataModel);
          setCflag(false);
        });
      ...
      const postOnClick = () => {
        CashCheckMoneyOrderService.savePostTransactionForCashCheckAndMoneyOrder(paymentbatch,paymentbatchtype).then(res => {
            ...
            <Col>
                ...
                {paymentbatchtype !== 0 ?
                    (<><Col>Payment Batch</Col>
                        ...
                        size="sm"
                        value={paymentbatch}
                        onChange={e => setPaymentBatch(Number(e.target.value))}
                        className="me-2"
                        >
        ...
        <Card className="bg-100 shadow-none border p-card">
            <Row className="g-3">
                ...
                <Col lg={{ span: 5, order: 1 }}>
                    {paymentbatch !== 0 ? <Payment data={loanData} chooseLoan={chooseLoan} /> : <></>}
                </Col>
                <Col lg={{ span: 5, order: 3 }}>
                    {paymentbatch !== 0 ? <Deposits data={depositData} chooseDeposit={chooseDeposit} /> : <></>}
                </Col>
            </Row>
        </Card>
        </>
