...
import {
  DebitCreditCardPaymentBatchService,
  DebitCreditCardService,
...
const AccountDetails = () => {
  ...
  useEffect(() => {
    DebitCreditCardPaymentBatchService.getGetPaymentBatchTypeSelection().then(
      res => setPaymentBatchTypes(res)
    );
  }, []);
  // same api from cashcheck
  useEffect(() => {
  ...
  return (
    <>
      <Row className="g-3 mb-3">
        <Col>
          <TitleCard title="Payment Reconciliation &gt; Debit Card/Credit Card" />
        </Col>
      </Row>
      <Row className="g-3 mb-3">
        <Col>
